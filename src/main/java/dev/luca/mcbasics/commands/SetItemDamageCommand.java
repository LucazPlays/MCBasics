...
public class SetItemDamageCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length != 1) {
            sender.sendMessage(Message.getComponent("setitemdamage.usage", "<gradient:#ff6b6b:#ee5a24>✖ Usage: /setitemdamage <damage></gradient>"));
            return true;
        }
        ...
        if (item == null || item.getType() == Material.AIR) {
            sender.sendMessage(Message.getComponent("setitemdamage.no_item", "<gradient:#ff6b6b:#ee5a24>✖ You must hold an item!</gradient>"));
            return true;
        }

        ItemMeta meta = item.getItemMeta();
        if (!(meta instanceof Damageable)) {
            sender.sendMessage(Message.getComponent("setitemdamage.not_damageable", "<gradient:#ff6b6b:#ee5a24>✖ This item cannot take durability damage!</gradient>"));
            return true;
        }
        ...
        try {
            ...
            sender.sendMessage(Message.getComponent("setitemdamage.invalid", "<gradient:#ff6b6b:#ee5a24>✖ Damage must be a number!</gradient>"));
            return true;
        }

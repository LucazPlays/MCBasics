    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("@a")) {
            ...
            for (Player player : Bukkit.getOnlinePlayers()) {
                ...
                player.sendMessage(FormattedMessage.create("heal.healed", "<gradient:#48dbfb:#1dd1a1>✦ You have been healed!</gradient>"));
                count++;
            }

            sender.sendMessage(FormattedMessage.create("heal.healed_all", "<gradient:#48dbfb:#1dd1a1>✦ All players (%count%) have been healed!</gradient>", "count", String.valueOf(count)));
            return true;
        }
        ...
        if (target != sender) {
            sender.sendMessage(FormattedMessage.create("heal.healed_other", "<gradient:#48dbfb:#1dd1a1>✦ %target% has been healed!</gradient>", "target", target.getName()));
        }

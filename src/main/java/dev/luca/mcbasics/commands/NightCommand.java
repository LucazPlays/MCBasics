...
public class NightCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            player.sendMessage(FormattedMessage.create("night.set", "<gradient:#48dbfb:#1dd1a1>✦ Time set to night!</gradient>"));
        } else {
            sender.getServer().getWorlds().forEach(world -> world.setTime(13000));
            sender.sendMessage(FormattedMessage.create("night.set", "<gradient:#48dbfb:#1dd1a1>✦ Time set to night!</gradient>"));
        }

...
public class DayCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            player.sendMessage(FormattedMessage.create("day.set", "<gradient:#48dbfb:#1dd1a1>✦ Time set to day!</gradient>"));
        } else {
            sender.getServer().getWorlds().forEach(world -> world.setTime(1000));
            sender.sendMessage(FormattedMessage.create("day.set", "<gradient:#48dbfb:#1dd1a1>✦ Time set to day!</gradient>"));
        }
